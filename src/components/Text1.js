...
const Text1 = ({
  ...
  return (
    <div>
      <div
        className={`w-[440px] my-0 mx-[!important]  mq750:left-20 mq450:left-0 rounded-mini ${display} flex-col items-start justify-start p-8 box-border gap-[32px] bg-cover bg-no-repeat bg-[top] max-w-full z-[1] text-left text-5xl text-white font-montserrat`}
        style={textStyle}
      >
        ...
        <button className="cursor-pointer [border:none] text-white py-2 px-5 bg-secondary rounded-full flex flex-row items-center justify-center whitespace-nowrap z-[1] hover:bg-darkgoldenrod">
          <img className=" w-6 z-[2]" loading="eager" alt="" src="/arrow.svg" />
        </button>
      </div>
    </div>
  );
